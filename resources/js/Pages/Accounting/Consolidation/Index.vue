...
function submitForm() {
  ...
  const ids = form.member_organization_ids
    .split(',')
    .map(s => s.trim())
    .filter(Boolean)
  ...
  <AppLayout :title="t('consolidation')">
    ...
    <Modal :open="showForm" :title="t('consolidation')" @close="showForm = false">
      <form class="space-y-4" @submit.prevent="submitForm">
        ...
        />
        <FormInput
          id="member_organization_ids"
          v-model="form.member_organization_ids"
          :label="t('consolidation_members')"
          :error="form.errors.member_organization_ids"
